  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: "13px", color: "var(--accent)", fontFamily: "var(--mono)", fontWeight: 600 }}>
          ✓ READY
        </span>
        <span style={{ fontSize: "12px", color: "var(--muted)", fontFamily: "var(--mono)" }}>
          {wordCount} words · {chunks} chunk{chunks !== 1 ? "s" : ""}
        </span>
      </div>
      ...
      {urls.map((url, i) => (
        <div key={url} style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          ...
          <audio controls src={url} style={{ width: "100%", height: "36px" }} />
            ...
            href={url}
            download
            style={{ fontSize: "12px", color: "var(--accent2)", fontFamily: "var(--mono)", textDecoration: "none" }}
          >
            {chunks > 1 ? "↓ download part " + (i + 1) : "↓ download"}
          </a>
        </div>
      ))}
    </div>
